(function () {
	...
	function getPreview(query, content, previewLength) {
		...
		if (match >= 0) {
			...
			preview = preview.replace(new RegExp("(" + parts.join("|") + ")", "gi"), "<span class='search-result-keyword'>$1</span>");
		} else {
	...
	function displaySearchResults(results, query) {
		...
		if (results.length) {
			...
			results.forEach(function (result) {
				...
				resultsHTML += "<li><h4 class='search-result-list'><a href='" + item.url.trim() + "'>" + titlePreview + "</a></h3><p><small>" + contentPreview + "</small></p></li>";
			});
